...
export async function sendOtp(phone: string): Promise<SendOtpResult> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Termii API error ${response.status}: ${body}`);
      return {
        success: true,
        code,
        message: "OTP generated (SMS delivery failed — using fallback)",
        fallback: true,
      };
    }
    ...
    console.error(`Termii send failed: ${errMsg}`);
    return {
      success: true,
      code,
      message: "OTP generated (SMS delivery failed — using fallback)",
